...
* add "tahoe list-aliases --readonly-uri" PR400 (`meejah`_)
* web: return "Referrer-Policy: no-referrer" on all requests (`meejah`_, `lew`_)
* deps: use stdlib "json" instead of external "simplejson" #2766 `PR405`_ (`meejah`_)
* add "tahoe status" command (`meejah`_)
* other: `PR407`_ `PR412`_ `PR410`_ `PR419`_ `PR423`_
* deps: require txi2p>=0.3.2 to work around TLS who-is-client issue #2861 `PR409`_
* complain more loudly in setup.py under py3 `PR414`_  (`tpltnt`_)
* immutable upload now uses happiness-metric algorithm #1382 `PR416`_ (`meejah`_, `markberger`_, `david415`_)
* deps: now need libyaml-dev from system before build PR420 (`ValdikSS`_)
* rename "filesystem" to "file store" #2345 `PR380`_ (`daira`_, `meejah`_)
* test "tahoe list-aliases --readonly-uri" #2863 `PR403`_ (`meejah`_)
* replace deprecated twisted.web.client with treq #2857 `PR428`_ (`warner`_)
* web: add JSON welcome page #2476 PR430 (`david415`_, `exarkun`_, `warner`_)
...
Developers may be excited about these non-user-visible changes:
