...
class AccountingCrawlerTest(unittest.TestCase, CrawlerTestMixin, WebRenderingMixin, ReallyEqualMixin):
    ...
    def test_expire_age(self):
        ...
        webstatus = StorageStatus(server, accountant)
        # we wait at the end for at least one complete cycle
        crawler_done = ac.set_hook('yield')
        ...
        def _after_first_cycle(ignored):
            ...
            for si in [immutable_si_1, mutable_si_3]:
                anon = yield sa.get_leases(si)
                self.failUnlessEqual(len(anon), 0)
                starter = yield aa.get_leases(si)
                self.failUnlessEqual(len(starter), 1)
